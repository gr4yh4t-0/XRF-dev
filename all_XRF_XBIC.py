...
scan439 = {'Scan #': 439, 'Name': 'TS58A XBIC', 'stanford': 200, 'lockin': 20, 'PIN beam_on': 225100, 'PIN beam_off': 624} #where the hell did we record these electrical settings... if 500 doesn't work, try 100. 
...
def generate_scalar_factor(scan_list):
    ...
    for scan in scan_list:
        correction = ((scan['stanford']*(1*10**-9)) / (beamconversion_factor * scan['lockin'])) * (1*10**9) #calculate correction factor for chosen scan
        correction_factors.append(correction)
        key = 'Scan ' + str(scan['Scan #']) + ' XBIC scale Factor'                     #define key for scan dictionary
        scan.setdefault(key, correction)                        #add key and correction factor to scan
    return correction_factors

correction_factors = generate_scalar_factor(scan_list)
