...
scanlist = {'475'};     % these are sector 2; add sector 26 scans %must be a 3digit number
datelist = {'Dec 17'};
%Corresponding names of samples for each scan
samplename={'NREL_NBL3-3'};
electtype= {'XBIC'};
...
%XBIC Data Corrections
%amp_setting = 1E-9; %sensitivity A/V
%cts_V = 100000;     %cts/V amplifier conversion
%q = 1.6E-19; %C/e- elemental charge
%c = 3.2; %carriers generated per incident photon based on the bandgap 
%flux = 1E9; %photon flux at 2-id-d. 1E9 is an estimate we have it measured but have not processed that data yet.
%xrf.XBIC_corr = amp_setting/cts_V/q/(c*flux)*100; % 1/count -- convert xbic raw to coll. eff.
...
% Cl Data
MW_Cl = 35.453;
WORK.Cl.map_corr = WORK.Cl.map; 
WORK.Cl.arr_corr = WORK.Cl.raw;
WORK.Cl.arr_mol  = WORK.Cl.arr_corr/MW_Cl*1E-6; 
WORK.Cl.map_mol  = WORK.Cl.map_corr/MW_Cl*1E-6; 
