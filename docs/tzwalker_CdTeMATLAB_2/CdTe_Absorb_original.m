...
%SodaLime (CaHNaO2) glass correction  % include double element presence?
mu_CaHNaO2_MoL = 746.8504; %cm2/g
mu_CaHNaO2_ZnK = 70.8730; %cm2/g
mu_CaHNaO2_TeL = 175.2544; %cm2/g
mu_CaHNaO2_CuK = 82.231; %cm2/g
mu_CaHNaO2_CdL = 282.7302; %cm2/g
mu_CaHNaO2_SK = 735.9339; %cm2/g
mu_CaHNaO2_SnL = 230.0583; %cm2/g
...
x_Mo = 5.0E-5;              %cm
x_ZnTe = 3.75E-5;           %cm   
x_loCu = 2.5E-7;              %cm          MUST CHANGE FOR EACH CdTe SAMPLE
x_CdTe = 5.0E-4;              %cm
x_CdS = 8E-6;            %cm
...
M = x_CdTe/dt;              %divide absorber thickness by number of steps...?
M = round(M);               %integer the layer division
depth = 0:dt:x_CdTe;        %1nm increments

%incident absorption
stack = {'Mo', 'ZnTe', 'loCu', 'CdTe', 'CdS', 'SnO2'};% 'SLG'};
...
cd3 = exp(-mu_Cu_CdL*p_Cu*x_loCu/rad_det); 
...
for i = 1:length(depth)
        ...
        beam_out = -p_CdTe*mu_CdTe_CdL*depth(i)/rad_det; %(check 1-sin?)
...
te3 = exp(-mu_Cu_TeL*p_Cu*x_loCu/rad_det); 
...
for i = 1:length(depth)
    ...
    i_io_cu(i) = ceo*exp(sum(beam_in+beam_out));
end
fCu = sum(i_io_cu)/M;
end
